...
if ( !function_exists( 'unicaevents_template_header_4_theme_setup' ) ) {
	add_action( 'unicaevents_action_before_init_theme', 'unicaevents_template_header_4_theme_setup', 1 );
	function unicaevents_template_header_4_theme_setup() {
		unicaevents_add_template(array(
			'layout' => 'header_4',
			'mode'   => 'header',
			'title'  => esc_html__('Header 4', 'unicaevents'),
			'icon'   => unicaevents_get_file_url('templates/headers/images/4.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'unicaevents_template_header_4_output' ) ) {
	function unicaevents_template_header_4_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_4 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_4 top_panel_position_<?php echo esc_attr(unicaevents_get_custom_option('top_panel_position')); ?>">
